...
public class CustomTrackVolumeStripController : SceneObjectScript

{
    ...
    private void ExecuteInteraction(InteractionData idata)
    {
        ...
        int volumePosition = (int)Math.Round(pct * 100, 0);
        //Log.Write("volumePosition: " + volumePosition);
        sendVolumeStripPosition(VolumeStripNumer, volumePosition, idata);

    }

    private void sendVolumeStripPosition(int volumeStripNumberIn, int volumePositionIn, InteractionData idata)
    {
        VolumeStripInfo sendVolumePosition = new VolumeStripInfo();
        sendVolumePosition.TrackStripNumber = volumeStripNumberIn;
        sendVolumePosition.TrackStripPosition = volumePositionIn;
        //Log.Write("TrackStripNumber: " + volumeStripNumberIn + " volumePosition: " + volumePositionIn);

        PostScriptEvent(ScriptId.AllScripts, "TrackVolume", sendVolumePosition);
    }
