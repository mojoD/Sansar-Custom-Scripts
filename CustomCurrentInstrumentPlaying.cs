...
public class CustomCurrentInstrumentPlaying : SceneObjectScript

{
    #region ConstantsVariables
    public readonly Interaction CurrentInstrumentInteraction;

    public bool Debug = false;

    private string OldInstrumentPlaying = null;
    private string CurrentInstrumentPlaying = null;
    ...
    {
        Log.Write("In Current Instrument Playing");
        ...
        SubscribeToScriptEvent("CurrentInstrument", getCurrentInstrument);  // Used to display sample name in prompt
        ComplexInteractionHandler();
    }

    private void ComplexInteractionHandler()
    {
        CurrentInstrumentInteraction.Subscribe((InteractionData idata) =>
        {
    ...
    private void getCurrentInstrument(ScriptEventData gotCurrentInstrument)
    {
        ...
        if (OldInstrumentPlaying != null)
        {
            SimpleData sd = new SimpleData(this);
            sd.SourceObjectId = ObjectPrivate.ObjectId;
            sd.AgentInfo = null;  //ScenePrivate.FindAgent.
            sd.ObjectId = ObjectId.Invalid;
            SendToAll(OldInstrumentPlaying + "Off", sd);
        }

        CurrentInstrumentPlaying = sendCurrentInstrument.CurrentInstrument;

        CurrentInstrumentInteraction.SetPrompt(CurrentInstrumentPlaying);

        OldInstrumentPlaying = CurrentInstrumentPlaying; 
    }
