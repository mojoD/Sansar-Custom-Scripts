...
public class CustomMediaController2 : SceneObjectScript

{
    #region ConstantsVariables
    [DefaultValue(".")]
    public readonly Interaction ComplexInteraction;
    public List<string> R1Streams = new List<string>();
    public List<string> R2Streams = new List<string>();
    public List<string> R3Streams = new List<string>();
    public List<string> R4Streams = new List<string>();
    public List<string> R5Streams = new List<string>();
    ...
    string SelectedStreamName = null;
    ...
    {
        Log.Write("In CustomMediaController");
    ...
    /*
    public class SendStreamInfo : Reflective
    {
        public string StreamToSend { get; set; }
    }
    */
        ...
        do
        {
            ...
            if (pointInRectangle)
            {
                ...
                if (hitZRelative >= ControlSurfaceZMinimum[cntr] && hitZRelative <= ControlSurfaceZMaximum[cntr])
                {
                    ...
                    GetStreamInfo(hitControlSurface, idata);
                    break;
                }
            }
            cntr++;
    ...
    private void GetStreamInfo(string controlSurface, InteractionData idata)
    {
        ...
        if (row == "R1")
        {
            SelectedStreamName = R1Streams[intColumn];
        }
        else if (row == "R2")
        {
            SelectedStreamName = R2Streams[intColumn];
        }
        else if (row == "R3")
        {
            SelectedStreamName = R3Streams[intColumn];
        }
        else if (row == "R4")
        {
            SelectedStreamName = R4Streams[intColumn];
        }
        else if (row == "R5")
        { 
            SelectedStreamName = R5Streams[intColumn];
        }
        else if (controlSurface != null) sendSimpleMessage(controlSurface, idata);  //Change Sample Pack

        //Log.Write("Selected StreamIn: " + SelectedStreamName);
        //Log.Write("row: " + row);
        if ((SelectedStreamName != null) && (row != "OS"))
        {
            //Jammer = ScenePrivate.FindAgent(idata.AgentId)?.AgentInfo;
            ParseVideo(SelectedStreamName);
            //TrackSelected = false;
        }
    }

/*
    private void SendStreamSelected(string StreamToSendIn)
    {
        SendStreamInfo sendStreamInfo = new SendStreamInfo();
        sendStreamInfo.StreamToSend = StreamToSendIn;
        PostScriptEvent(ScriptId.AllScripts, "Stream", sendStreamInfo);
        Log.Write("CustomMediaController - StreamToSendIn: " + StreamToSendIn);
    }
*/
    ...
    private void ParseVideo(string videoIn)
    {
        //Log.Write("Video into Parse Video: " + videoIn);
        IsWatchFormat = false;
        video = videoIn;
        videoIn = video;
        //Log.Write("video: " + video);
        if (videoIn.Contains("/watch?v="))
        {
            IsWatchFormat = true;
            video = URLToEmbedFormat(videoIn);
            //Log.Write("New Video: " + video);
        }
        if (videoIn.Contains("Youtu.be"))
        {
            IsWatchFormat = true;
            video = ShortenedURLToEmbedFormat(videoIn);
            //Log.Write("New Video: " + video);
        }
        PlayVideo(video);
    }
