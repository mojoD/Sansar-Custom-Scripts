...
public class CustomOneShotPadController : SceneObjectScript

{
    ...
    {
        //Log.Write("In CustomOneShotController");
    ...
    private void GetSampleInfo(string controlSurface, InteractionData idata)
    {
        ...
        else if (controlSurface != null) sendSimpleMessage(controlSurface, idata);  //Change Screen

        if ((SelectedSampleName != null) && (row != "OS"))
        {
            //Jammer = ScenePrivate.FindAgent(idata.AgentId)?.AgentInfo;
            PlaySampleSelected(SelectedSampleName, SelectedSoundResource, DefaultPan, SelectedTrackIn, Jammer);
    ...
    private void PlaySampleSelected(string sampleNameIn, SoundResource sampleSoundResourceIn, string TrackPanIn, string trackToUseIn, AgentInfo Jammer)
    {
        //Log.Write("A - Sample: " + sampleSoundResourceIn.GetName());

        //Log.Write("B");
        audio.PlaySoundOnComponent(sampleSoundResourceIn, playSettings);
        //Log.Write("C");
    }
